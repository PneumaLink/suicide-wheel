...
const GamePage = (
  ...
  // 쌓으려는 스택 값
  const [targetUserStack, setTargetUserStack] = useState(1);
  ...
  const [finalList, setFinalList] = useState<{name: string; stack: number;}[]>([])
            ...
            <label>당첨 확률을 높이고 싶은 대상을 선택해주세요.</label>
            {
              canSelectUserList.map((item, index) => {
                return(
                  <div key={index}>
                    <button 
                      name="user_name_button"
                      onClick={()=>{
                        setTargetUser(item.name)
                      }} 
                      className={`button ${targetUser === item.name ? 'is-success is-light' : ''}`}
                    >
                      {item.name}
                    </button>
                  </div> 
            ...
            <div>
              <button 
                onClick={()=>{
                  ...
                  setFinalList(selectRandomItems(userList, 1));
                }}
                className={"button"}
                >
                돌려돌려 돌림판!
            ...
            <div>
              {
                finalList.map((item, index) => {
                  return (
                    <div>
                      <label key={index}>당첨자 {index+1} : {item.name}</label>
                    </div>
                  )
                })
              }
